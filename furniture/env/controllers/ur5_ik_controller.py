...
class Ur5IKController(Controller):
    ...
    def setup_inverse_kinematics(self):
        ...
        self.robot_urdf = pjoin(
            self.bullet_data_path, "ur_description/urdf/ur5_arm.urdf"
        )
    ...
    def ik_robot_eef_joint_cartesian_pose(self):
        ...
        """
        eef_pos_in_world = np.array(p.getLinkState(self.ik_robot, 7)[0])
        eef_orn_in_world = np.array(p.getLinkState(self.ik_robot, 7)[1])
        ...
        if rest_poses is None:
            ik_solution = list(
                p.calculateInverseKinematics(
                    ...
                    restPoses=[0, -np.pi/4, np.pi/2, -3*np.pi/4, -np.pi/2,  np.pi/2],
                    jointDamping=[0.1] * 9,
                )
            )
        else:
            ik_solution = list(
                p.calculateInverseKinematics(
                    ...
                    target_position,
                    targetOrientation=target_orientation,
                    lowerLimits=[-6.28318530718, -6.28318530718, -3.14159265359, -6.28318530718, -6.28318530718, -6.28318530718],
                    upperLimits=[6.28318530718, 6.28318530718, 3.14159265359, 6.28318530718, 6.28318530718, 6.28318530718],
                    jointRanges=[12.5, 12.5, 6.2, 12.5, 12.5, 12.5],
    ...
    def joint_positions_for_eef_command(self, dpos, rotation):
        ...
        rest_poses = [0, -np.pi/4, np.pi/2, -3*np.pi/4, -np.pi/2,  np.pi/2]
